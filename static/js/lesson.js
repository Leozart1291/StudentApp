...
function initializeCamera() {
    ...
    async function startCamera() {
        try {
            ...
            // Скрыть placeholder и показать controls
            ...
            // Отправлять кадр камеры на сервер для учеников (каждые 150 мс)
            if (typeof lessonAccessCode !== 'undefined' && lessonAccessCode) {
...
async function loadGestureVideos() {
    try {
        ...
        if (data.success && data.videos) {
            ...
            gestureWords = data.videos.map(function(v) { return v.text; });
            ...
            recognition.onresult = function(event) {
                ...
                if (newFinalText.trim()) {
                    ...
                    if (normalizedNewText !== normalizeText(lastProcessedText)) {
                        ...
                        var words = normalizedNewText.split(' ').filter(function(w) { return w.length > 0; });
...
function initializeTaskTests() {
    document.querySelectorAll('.task-test').forEach(function(block) {
        ...
        function getCorrectIndex() {
            ...
            const found = options.findIndex(function(o) { return getOptionText(o, 0) === correctStr; });
